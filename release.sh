...
MAIN_BRANCH="master"
ANNOTATION=""
REPO="MX-Linux/mx-conky"

# Helper functions
print_header() {
    echo -e "${BLUE}========================================${NC}"
    echo -e "${BLUE}  MX Conky Arch Release Script${NC}"
    echo -e "${BLUE}========================================${NC}"
    echo
}
...
update_aur_package() {
    local version=$1
    local annotation=$2
    local pkgver_version=${version#v}
    ...
    if [ -f PKGBUILD ]; then
        print_step "Updating PKGBUILD pkgver to $pkgver_version..."
        if rg -q "^pkgver=" PKGBUILD; then
            sed -i "s/^pkgver=.*/pkgver=${pkgver_version}/" PKGBUILD
        else
            awk -v ver="$pkgver_version" '
